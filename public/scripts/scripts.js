        ...
        for (let R = cell_range.s.r; R <= cell_range.e.r; ++R) {
          for (let C = cell_range.s.c; C <= cell_range.e.c; ++C) {
            ...
            if (cell && headerArray.includes(cell.v.toString().trim())) {
              let colHeader_address = {
                c: C,
                r: R + 1
              }

              let colHeader = XLSX.utils.encode_cell(colHeader_address)
              let cellValue = sheet[colHeader]
    ...
    if (arrestmentArray[i][1] === asfArray[j][0]) {
      for (k = 0; k < asfArray[j].length; k++) {
        ...
        if (diff > 0.2) {
          ...
          diffCell += "<td bgcolor=lightblue>" + diff + "</td>"
        }
      }
...
function readExcelFile(targetedFile, targetedRange, resultArray) {

  let reader = new FileReader();
  reader.readAsArrayBuffer(targetedFile);
  ...
  reader.onload = function (e) {
    ...
    let cell_range = XLSX.utils.decode_range(targetedRange) // get the desired range only
